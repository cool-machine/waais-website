...
7. Run `php artisan migrate --force`. Not done — first production migration is the next slice. The GitHub runner cannot reach PostgreSQL through the firewall (which allows only Azure services), so run via `az webapp ssh -c 'cd /home/site/wwwroot && php artisan migrate --force'` once.

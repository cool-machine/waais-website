...
Implement **email notifications**. Reuse the same notification surfaces across membership applications and startup listings: (1) applicant/submitter thank-you on submission; (2) admin new-submission notice (one email per admin? or a configurable inbox?); (3) approval email; (4) request-more-info email; (5) rejection email is optional and admin-triggered (a flag on the reject endpoint). Use Laravel's `Notification` system with a single mailable per event. Email provider is still TBD — start with the `log` mail driver in dev so we don't block on provider choice; the production target is likely Azure Communication Services Email or Google Workspace. Add a feature test per notification using `Notification::fake()`. After this slice, the next priority is wiring the Vue public startup directory (`frontend/src/data/startups.js` → `/api/public/startup-listings`) so the public site stops running on static seed data.
```

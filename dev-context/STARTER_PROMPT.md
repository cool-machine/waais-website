...
**Next slice — production Google OAuth.** The production frontend at `https://whartonai.studio` shows the "Sign in with Google" button on `/app/sign-in` and the membership flow needs it, but Google OAuth client credentials are not yet provisioned for the production hostname. Steps: (1) create a new OAuth 2.0 Client ID in Google Cloud Console (or extend the existing dev client) with authorized JavaScript origins `https://whartonai.studio` and authorized redirect URIs `https://api.whartonai.studio/auth/google/callback`; (2) write the resulting client ID and secret into App Service application settings as `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`, plus `GOOGLE_REDIRECT_URI=https://api.whartonai.studio/auth/google/callback` (verify the exact var names match what `backend/config/services.php` reads); (3) restart the App Service so the new settings load; (4) smoke a real sign-in from `https://whartonai.studio/app/sign-in` end-to-end. After this, remaining steps are: ACS Email setup for transactional mail, scheduler runner for `php artisan schedule:run` (likely an Azure Container Apps job or a small App Service WebJob), then the final smoke checks in `AZURE_PRODUCTION.md`. Discourse remains deferred to the final stage.
```

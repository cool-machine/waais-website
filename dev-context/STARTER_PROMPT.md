...
- Laravel Sanctum API auth foundation is present, including /api/user and member-only API middleware. Google OAuth backend foundation is present, including pending-user provisioning.
- Applicant-owned membership application APIs are present for show, submit, update, and rejected-applicant reapply.

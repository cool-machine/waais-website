...
Wire the **Vue public startup directory** to the live API. Replace the static seed in `frontend/src/data/startups.js` with calls to `GET /api/public/startup-listings` (paginated, default `per_page=12`, capped at 48) and `GET /api/public/startup-listings/{id}`. Keep the existing `StartupsPage.vue` and detail-page shape — only the data source changes. Surface a loading state, an empty state for "no published listings yet," and a graceful error fallback. Do not introduce auth on these calls; both endpoints are anonymous. The response projection is documented in `backend/README.md` ("Public Startup Listing API Shape"); the Vue layer should consume only those fields. After that, the next priority is the events / partners / homepage CMS APIs (same Submission & Admin Review Pattern, plus event-specific lifecycle for capacity, waitlist, cancellation, recap, and reminders).
```

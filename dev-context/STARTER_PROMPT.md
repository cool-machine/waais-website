...
Implement the **public read API for published startup listings**. Add `/api/public/startup-listings` (index) and `/api/public/startup-listings/{listing}` (show) — no authentication required. Both endpoints must filter to `content_status = published` AND `visibility = public` only; anything else returns 404. The shape should be a trimmed projection of `StartupListing` (no review_notes, no reviewed_by, no internal timestamps). This is the slice that finally lets the Vue frontend stop running on static seed data, so keep the response shape stable and document it in `backend/README.md`. After this slice, the next priority is email notifications (applicant thank-you, admin new-submission notice, approval, request-info) reusing the same templates across membership applications and startup listings.
```

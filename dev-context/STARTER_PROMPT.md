...
**In-flight slice (`slice/frontend-prod-deploy`):** wiring the production frontend deploy to `swa-waais-prod-weu` (Azure Static Web App, Free tier, default host `proud-moss-0ec457703.7.azurestaticapps.net`, no GitHub integration yet) and binding `whartonai.studio` apex with a managed cert. Plan: (1) pull SWA deployment token via `az staticwebapp secrets list` and add as `AZURE_STATIC_WEB_APPS_API_TOKEN` GitHub secret; (2) commit `.github/workflows/deploy-frontend.yml` that runs `npm ci && npm run build` with `VITE_API_BASE_URL=https://api.whartonai.studio`, then deploys via `Azure/static-web-apps-deploy@v1`; (3) push, watch the workflow, smoke the SWA default host; (4) ask George to update Cloudflare apex DNS — replace the current proxied A records (which point at the GH Pages preview through Cloudflare) with a CNAME-flattened `@` → `proud-moss-0ec457703.7.azurestaticapps.net` (DNS only / proxy off) plus the SWA-issued validation TXT (the validation token comes from `az staticwebapp hostname set` when bound in DNS-token mode); (5) bind via `az staticwebapp hostname set --hostname whartonai.studio --validation-method dns-txt-token`; (6) wait for SWA's managed cert to issue automatically; (7) smoke `https://whartonai.studio` end-to-end. Keep GitHub Pages as a preview path — don't disable that workflow.

After this slice ships, next steps remain: production Google OAuth client and the `GOOGLE_*` settings, ACS Email setup, scheduler runner for `php artisan schedule:run`, then the smoke checks in `AZURE_PRODUCTION.md`. Discourse remains deferred to the final stage.
```
